...
const OurSpecs = () => {
    ...
    return (
        <div className='px-3 sm:px-6 my-16 sm:my-20 max-w-6xl mx-auto'>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-7 gap-y-8 sm:gap-y-10 mt-16 sm:mt-26'>
                {
                    ourSpecsData.map((spec, index) => {
                        return (
                            <div className='relative h-36 sm:h-44 px-4 sm:px-8 flex flex-col items-center justify-center w-full text-center border rounded-lg group' style={{ backgroundColor: spec.accent + 10, borderColor: spec.accent + 30 }} key={index}>
                                ...
                                <div className='absolute -top-4 sm:-top-5 text-white size-8 sm:size-10 flex items-center justify-center rounded-md group-hover:scale-105 transition' style={{ backgroundColor: spec.accent }}>
                                    <spec.icon size={16} className="sm:hidden" />
                                    <spec.icon size={20} className="hidden sm:block" />
                                </div>
                            </div>
                        )
                    })
